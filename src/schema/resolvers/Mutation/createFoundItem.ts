...
export const createFoundItem: NonNullable<MutationResolvers['createFoundItem']> = async (_parent, _arg, _ctx) => {
  const recentItems = await _ctx.FoundItem.find({
    foundBy: new mongoose.Types.ObjectId(_arg.foundBy),
  ...
  const foundItem = new _ctx.FoundItem({
    title: _arg.title,
    description: _arg.description,
    location,
    foundBy: new mongoose.Types.ObjectId(_arg.foundBy),
  ...
  return foundItem.populate('foundBy');
};